<template>
  <view>
    <view id="myNavBar_add">
          <van-nav-bar  :safe-area-inset-top="false" title="我的地址"  leftArrow=true>
          </van-nav-bar>
    </view>
    ...
    <view class='myAddressList'>
      <button class="add_button" @tap="xx">新增地址</button>
    </view>
  </view>
</template>
...
export default {
  ...
  methods:{
    ...
    chooseAddress(){
      Pubsub.publish("chooseAdd123","zz");
      Taro.navigateTo({
        url: '../order/order',
      })
    },
    editAddress(index){
      Taro.navigateTo({
        url: '../address/editaddress?address_id='+this.list[index].address_id,
        // success: function (res) {
        //   // 通过eventChannel向被打开页面传送数据
        //   res.eventChannel.emit('acceptDataFromOpenerPage', { data: this.list[index].address_id })
        // }
      })
    }
  },
